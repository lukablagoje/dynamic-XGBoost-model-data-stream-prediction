...
import time
import matplotlib.pyplot as plt
def create_folds(number_of_folds,rolling_test_size_ratio,fold_size,df_full,stock_id):
    rolling_test_size = int(fold_size*rolling_test_size_ratio)
    df = df_full[df_full['stock_id'] == stock_id].copy()
...
alpha_list = [100]
min_child_list = [100]
eta_list = [0.001,0.01,0.1,0.25,0.5,0.75,1]
stock_id_list = sorted(list(df['stock_id'].unique()))
total_iter = len(max_depth_list) * len(n_estimators_list) * len(alpha_list) * len(min_child_list)*len(eta_list)*len(lambda_list)*len(stock_id_list)
result_mean_dict = {}
result_std_dict = {}
results_df_dict =  {'max_depth':[],'n_estimators':[],'eta':[],'min_child':[],'alpha':[],'lambda':[],'train_mean_mae':[],'train_std_mae':[],'test_mean_mae':[],'test_std_mae':[],'stock_id':[]}
count = 1

print('Hyperparameter evaluation starting')
method_dict = {'model_name' : 'ind_stocks_xgboost',
'retraining_freq' : 'no_retraining',
'retraining_method' : 'no_data'}
for stock_id in stock_id_list:
    fold_dict,X_train_dict, y_train_dict,X_test_dict, y_test_dict = create_folds(number_of_folds,rolling_test_size_ratio,fold_size,df.copy(),stock_id)
                ...
                for eta in eta_list:
                    for n_estimators in n_estimators_list:
                        for max_depth in max_depth_list:
                                ...
                                test_mean = np.round(np.nanmean(mae_test_list.copy()),5)
                                test_std = np.round(np.nanstd(mae_test_list.copy()),5)
                                results_df_dict['test_mean_mae'].append(test_mean)
                                results_df_dict['test_std_mae'].append(test_std)
                                train_mean = np.round(np.nanmean(mae_train_list.copy()),5)
                                train_std = np.round(np.nanstd(mae_train_list.copy()),5)        
